...
function initApp() {
    ...
    // Initialize search functionality - check if element exists first
    ...
    // Initialize periodic table interactions - check if elements exist first
    const elements = document.querySelectorAll('.element');
    if (elements.length > 0) {
        elements.forEach(element => {
            element.addEventListener('click', function() {
                const elementName = this.querySelector('.element-name');
                if (elementName) {
                    showToast('info', 'Element Info', `You clicked on ${elementName.textContent}`);
                }
            });
        });
    }
        ...
        if (snapshot.exists()) {
            ...
            userChallengeRef.once('value').then((userSnapshot) => {
                if (!userSnapshot.exists()) {
                    ...
                    const dailyChallengeQuestion = document.getElementById('dailyChallengeQuestion');
                    
                    if (dailyChallengeElement && dailyChallengeQuestion) {
                        dailyChallengeElement.style.display = 'block';
                        dailyChallengeQuestion.textContent = dailyChallenge.question;
                    }
                }
            });
        } else {
...
function showDailyChallengeModal() {
    ...
    const dailyChallengeQuestionText = document.getElementById('dailyChallengeQuestionText');
    const optionsContainer = document.getElementById('dailyChallengeOptions');
    
    if (dailyChallengeModal && dailyChallengeQuestionText && optionsContainer) {
        dailyChallengeModal.classList.add('active');
        dailyChallengeQuestionText.textContent = dailyChallenge.question;
        ...
        const submitBtn = document.getElementById('dailyChallengeSubmitBtn');
...
function showElementQuiz(element) {
    ...
    if (popup && title && questionText && optionsContainer && feedback) {
...
function showTermQuiz(term) {
    ...
    const modal = document.getElementById('quizQuestionModal');
    ...
    if (modal && title && questionText && answersContainer && feedback) {
...
function showTrendQuiz(trend) {
    ...
    if (modal && title && questionText && answersContainer && feedback) {
        modal.classList.add('active');
        title.textContent = `Trend Quiz: ${trend.replace('-', ' ')}`;
        questionText.textContent = question.question;
...
function showElementDetails(name, symbol, atomicNumber, electronConfig) {
    ...
    const nameElement = document.getElementById('elementDetailsName');
    const symbolElement = document.getElementById('elementDetailsSymbol');
    const numberElement = document.getElementById('elementDetailsNumber');
    const configElement = document.getElementById('elementDetailsConfig');
    const mpElement = document.getElementById('elementDetailsMP');
    const densityElement = document.getElementById('elementDetailsDensity');
    const reactivityElement = document.getElementById('elementDetailsReactivity');
    const usesElement = document.getElementById('elementDetailsUses');
    const factElement = document.getElementById('elementDetailsFact');
    const quizQuestionElement = document.getElementById('elementDetailsQuizQuestion');
    const optionsContainer = document.getElementById('elementDetailsQuizOptions');
    const feedbackElement = document.getElementById('elementDetailsQuizFeedback');
    
    if (modal && nameElement && symbolElement && numberElement && configElement && 
        mpElement && densityElement && reactivityElement && usesElement && factElement &&
        quizQuestionElement && optionsContainer && feedbackElement) {
        
        nameElement.textContent = name;
        symbolElement.textContent = symbol;
        numberElement.textContent = atomicNumber;
        configElement.textContent = electronConfig;
        mpElement.textContent = details.meltingPoint;
        densityElement.textContent = details.density;
        reactivityElement.textContent = details.reactivity;
        usesElement.textContent = details.uses;
        factElement.textContent = details.fact;
        
        // Setup quiz
        quizQuestionElement.textContent = details.quizQuestion;
        optionsContainer.innerHTML = '';
...
function showTrendExplanation(trend, element) {
    ...
    const titleElement = document.getElementById('trendExplanationTitle');
    const headingElement = document.getElementById('trendExplanationHeading');
    const contentElement = document.getElementById('trendExplanationContent');
    const quizQuestionElement = document.getElementById('trendExplanationQuizQuestion');
    const optionsContainer = document.getElementById('trendExplanationQuizOptions');
    const feedbackElement = document.getElementById('trendExplanationQuizFeedback');
    
    if (modal && titleElement && headingElement && contentElement && 
        quizQuestionElement && optionsContainer && feedbackElement) {
...
function showReactionDetails(reaction) {
    ...
    const titleElement = document.getElementById('reactionDetailsTitle');
    const equationElement = document.getElementById('reactionDetailsEquation');
    const observationsElement = document.getElementById('reactionDetailsObservations');
    const explanationElement = document.getElementById('reactionDetailsExplanation');
    const quizQuestionElement = document.getElementById('reactionDetailsQuizQuestion');
    const optionsContainer = document.getElementById('reactionDetailsQuizOptions');
    const feedbackElement = document.getElementById('reactionDetailsQuizFeedback');
    
    if (modal && titleElement && equationElement && observationsElement && 
        explanationElement && quizQuestionElement && optionsContainer && feedbackElement) {
        
        titleElement.textContent = `${reaction.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}`;
        equationElement.textContent = details.equation;
        observationsElement.textContent = details.observations;
...
function showReactionVideo(element) {
    ...
    const observationsElement = document.getElementById('reactionVideoObservations');
    if (observationsElement) {
        observationsElement.textContent = observations[element] || 'Observations not available.';
    }
...
function showOxideTrendQuiz() {
    ...
    if (modal && title && questionText && answersContainer && feedback) {
        modal.classList.add('active');
        title.textContent = 'Oxide Trend Quiz';
        questionText.textContent = question.question;
...
function startQuiz() {
    ...
    if (modal && progress && content && results) {
        modal.classList.add('active');
...
function addAntiCheatingMeasures() {
    ...
    if (modal) {
        ...
        modal.addEventListener('mousedown', e => e.preventDefault());
    }
...
function removeAntiCheatingMeasures() {
    ...
    const modal = document.getElementById('quizModal');
    if (modal) {
        modal.removeEventListener('contextmenu', e => e.preventDefault());
        
        // Re-enable text selection
        modal.removeEventListener('selectstart', e => e.preventDefault());
        modal.removeEventListener('mousedown', e => e.preventDefault());
    }
...
function handleFeedback(event) {
    ...
    if (!textElement || !ratingElement) {
        showToast('error', 'Error', 'Feedback form elements not found.');
        return;
    }
...
function showEditPostModal(postId) {
    ...
    const titleElement = document.getElementById('editPostTitle');
    const contentElement = document.getElementById('editPostContent');
    const tagsElement = document.getElementById('editPostTags');
    
    if (modal && postIdElement && titleElement && contentElement && tagsElement) {
...
function handleCreatePost(event) {
    ...
    if (!titleElement || !contentElement || !tagsElement) {
        showToast('error', 'Error', 'Post form elements not found.');
        return;
    }
...
function handleEditPost(event) {
    ...
    if (!postIdElement || !titleElement || !contentElement || !tagsElement) {
        showToast('error', 'Error', 'Edit post form elements not found.');
        return;
    }
...
function loadDiscussionPosts() {
    ...
    if (!postsContainer) {
        console.error("Discussion posts container not found");
        return;
    }
...
function handleCreateGroup(event) {
    ...
    const topicElement = document.getElementById('groupTopic');
    const descriptionElement = document.getElementById('groupDescription');
    const maxMembersElement = document.getElementById('groupMaxMembers');
    
    if (!nameElement || !topicElement || !descriptionElement || !maxMembersElement) {
        showToast('error', 'Error', 'Group form elements not found.');
        return;
    }
    ...
    const maxMembers = parseInt(maxMembersElement.value);
...
function loadStudyGroups() {
    ...
    if (!groupsContainer) {
        console.error("Study groups container not found");
        return;
    }
...
function showGroupDetails(groupId) {
    ...
    const modalContentElement = document.getElementById('groupDetailsContent');
...
function loadGraphQuiz() {
    ...
    const questionElement = document.getElementById('graph-quiz-question');
